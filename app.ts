...
class UI {
  addieentrytoList(ieentry: Ieentryinterface) {
    ...
    row.innerHTML = `
            ...
            `;
    if (ieentry.ie == "I") row.style.backgroundColor = "#67e467";
    else row.style.backgroundColor = "#e94f4f";
  ...
  filterfunc(
    filter: string // typed input value as asrgument
  ) {
    let filterby: number = +(document.getElementById(
      "filterby"
    ) as HTMLInputElement).value; //filterby as a number for comparing the right child ie,date,name,amount as 0,1,2 &3
    let rows = document.getElementById("ie-list")!.children;
    for (let i = 0; i < rows.length; i++) {
      let row = <HTMLElement>rows[i];
      if (filter == "") {
        // for the filterby change listner
        row.style.display = "";
      } else if (
        row.children[filterby].textContent!.toUpperCase().indexOf(filter) > -1
      ) {
        row.style.display = "";
      } else {
...
class LocalStore {
  static getieentriesfromLS(): Array<Ieentryinterface> {
    ...
    if (localStorage.getItem("ieentries") === null) {
      ...
      for (
        let i = 0;
        i < ieentriesdateasstring.length;
        i++ // upgrading the type of date from string to date
      ) {
  ...
  static removeieentry(ieentrytobedeleted: Ieentryinterface) {
    ...
    ieentries.forEach(function (ieentry, index) {
      if (
        found == false &&
        JSON.stringify(ieentry) === JSON.stringify(ieentrytobedeleted)
      ) {
        //for proper object comparison, date is a Date() from getieentriesfromLS()
...
class Data {
  static validateAndProcess(datainCSV: string) {
    ...
    if (datainCSV == "") {
      ui.showAlert(
        "Input is empty! Enter the value in CSV fromat with header",
        "error"
      );
      return false;
    }
    ...
    if (hd.length !== 4) {
      ui.showAlert(
        "Header is incorrect! Enter the value in CSV fromat with header",
        "error"
      );
      return false;
    }
    ...
    if (
      ...
      ui.showAlert(
        "Header is not correct! Enter the value in CSV fromat with header",
        "error"
      );
      return false;
    }
    ...
    if (arr.length == 1) {
      ui.showAlert(
        "Only header is present! Enter the value in CSV fromat below header",
        "error"
      );
      return false;
    }
    ...
    if (arr[arr.length - 1] == "") {
      ui.showAlert(
        "Last line is empty! Do not enter a linebreak after the last entry.",
        "error"
      );
      return false;
    }
    ...
    for (
      let i = 1;
      i < arr.length;
      i++ //validating each entry
    ) {
      let data = arr[i].split(",");
      for (
        let j = 0;
        j < 4;
        j++ //triming and checking for empty values
      ) {
        data[j] = data[j].trim();
        if (data[j] == "") {
      ...
      if (isNaN(+data[3])) {
        ...
        ui.showAlert(
          `Amount of entry number ${i} is not a valid number`,
          "error"
        );
        return false;
      }
...
document
  ...
  .addEventListener("click", function (e) {
    ...
    if (tar.className == "delete") {
      // ensuring the click is on the X
      const ui = new UI();
      //collecting all the values of the row to be deleted
      const amount = tar.parentElement!.previousElementSibling!.textContent;
      const name = tar.parentElement!.previousElementSibling!
        .previousElementSibling!.textContent;
      const date = tar.parentElement!.previousElementSibling!
        .previousElementSibling!.previousElementSibling!.textContent;
      const ie = tar.parentElement!.previousElementSibling!
      ...
      if (ie == "I") {
        //crosschecking with the ie value collected
...
document.getElementById("sortby")!.addEventListener("change", function (e) {
  ...
  const sortby: string = (document.getElementById("sortby") as HTMLInputElement)
    .value;
  //empty thetable
  ...
  let ui = new UI();
  if (ieentries.length == 0) {
    //verifying there is some data to be sorte
    ...
    if (sortby == "ie") {
      ...
      //for sort by time of entry. simply printing from ls, no sort involved
...
document.getElementById("submitbtn")!.addEventListener("click", function (e) {
  const datainCSV = (document.getElementById("ie-input") as HTMLInputElement)
    .value;
